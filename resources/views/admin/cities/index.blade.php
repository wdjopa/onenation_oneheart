...
@section("subtitle", "La liste des villes enregistrées")
            ...
            <div class="card-body">
                <div class="mb-4">
                    ...
                    <div class="text-right" style="text-align: right;">
                        <a href="{{route("cities.create")}}" class="btn btn-success">Ajouter une ville</a>
                    </div>
                </div>
                <div class="table-responsive">
                    <form action="{{route("cities.multipleDestroy")}}" method="post" name="deleteForm">
                        @csrf
                        <table class="table no-wrap v-middle mb-0" id="dataTable">
                            <thead>
                                <tr class="border-0">
                                    ...
                                    <th class="font-14 font-weight-medium text-muted">Pays</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <label class="custom-control custom-checkbox">
                                            <input class="custom-control-input checkboxes" type="checkbox"
                                                value="{{$city->id}}" name="ids[]" id="check{{$city->id}}"><span
                                    ...
                                    <td>
                                        <a type="button" class="btn btn-primary" href={{ route("cities.edit", $city) }}><i
                                                class="bi-pencil c-orange-700"></i></a>
                                        </a>
                                        <button type="button"
                                            onclick="document.querySelector('#check{{$city->id}}').setAttribute('checked', 'checked');(confirm('Êtes-vous sûr de vouloir supprimer cette(s) ville(s) ?')? document.deleteForm.submit():console.log('annulé'));"
