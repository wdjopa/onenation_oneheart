...
@section("subtitle", "Ajoutez un article ici.")
...
<div class="col-12">
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">Création d'un article</h4>
            <small>Tous les champs avec (*) sont obligatoires</small>
        </div>
        <div class="card-content">
            <div class="card-body">
                <form class="form form-horizontal" method="POST" action="{{route("blogs.update", $blog)}}"
                    enctype="multipart/form-data">
                    @csrf
                    @method("PUT")
                    @include("admin.blogs.components.form", ["fields" => $fields, "blog"=>$blog])
                </form>
            </div>
        </div>
    </div>
