...
{{-- Titre de la page --}}
@section('title')
    Articles de Blog / Actions menées
@endsection

@section('subtitle', 'La liste des articles enregistrés')
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <form action="{{ route('blogs.multipleDestroy') }}" method="post" name="deleteForm">
                            @csrf
                                            ...
                                            <td>
                                                <label class="custom-control custom-checkbox">
                                                    <input class="custom-control-input checkboxes" type="checkbox"
                                                        value="{{ $blog->id }}" name="ids[]"
                                                        id="check{{ $blog->id }}"><span
                                                        class="custom-control-label"></span>
                                                </label>
                                            </td>
                                            <td>{{ $blog->name }}</td>
                                            <td><a
                                                    href="{{ route('public.blog.details', ['blog_slug' => $blog->slug]) }}">{{ $blog->slug }}</a>
                                            </td>
                                            <td>{{ views($blog)->count() }}</td>
                                            <td>
                                                <a type="button" class="btn btn-primary"
                                                    href={{ route('blogs.edit', $blog) }}><i
                                                        class="bi-pencil c-orange-700"></i></a>
                                                </a>
                                                <button type="button"
                                                    onclick="document.querySelector('#check{{ $blog->id }}').setAttribute('checked', 'checked');(confirm('Êtes-vous sûr de vouloir supprimer cet(s) article(s) ?')? document.deleteForm.submit():console.log('annulé'));"
