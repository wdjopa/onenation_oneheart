    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Modification d'un partenaire</h4>
                <small>Tous les champs avec (*) sont obligatoires</small>
            </div>
            <div class="card-content">
                <div class="card-body">
                    <form class="form form-horizontal" method="POST" action="{{ route('partners.update', $partner) }}"
                        enctype="multipart/form-data">
                        @csrf
                        @method("PUT")
                        @include("admin.partners.components.form", ["fields" => $fields, "partner" => $partner])
                    </form>
                </div>
            </div>
        </div>
